; packet receive ratio based on distance.
; setting r-counter decision on the ide
; adding coverege percentage

breed [sensor a-sensor]
sensor-own [r-counter]

globals[msg-no covered]
...
to setup
  ...
  create-sensor 1 [  ;; create the initial sheep
    setxy 0 0
    set color red
    set shape "circle"
    set r-counter 1
...
to go
  ask sensor with [color = red and r-counter <= r-counter-max] [
    ask sensor in-radius communication-range [
      let d distance myself
      let p (communication-range - d) / communication-range
      if random 100 < 100 * p [         ;p
        set color red               ;p
        set r-counter r-counter + 1 ;p
      ]                             ;p
    ]
    set color blue
    set msg-no msg-no + 1
  ]
  if not any? sensor with [color = red and r-counter <= r-counter-max] [stop] ;added line
  set covered 100 * (num-nodes - (count sensor with [color = white])) / num-nodes
  tick
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot msg-no"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot covered"
